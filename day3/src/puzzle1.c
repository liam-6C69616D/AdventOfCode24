...
int main() {
    ...
    const char *pattern = "mul\\([0-9]{1,3},[[:space:]]*[0-9]{1,3}\\)";  // Match mul(num, num)

    // Call function to find all matches

    size_t matchCount = 0;
    Match* matches = find_all_matches(fileContents, pattern, &matchCount);
    
    int sum = 0;
    for (int i=0; i<matchCount; i++) {
        int length = matches[i].end - matches[i].start;

        // Allocate memory for the substring
        char substring[length + 1]; // +1 for the null terminator

        // Copy the substring to the new variable using strncpy
        strncpy(substring, fileContents + matches[i].start, length);
